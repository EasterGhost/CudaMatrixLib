...
__global__ static void convert_kernel(const data_type1 *src, data_type2 *dst, int size)
{
	int idx = threadIdx.x + blockIdx.x * blockDim.x;
	if (idx < size)
	{
		dst[idx] = (data_type2)src[idx];
	}
}

template <typename data_type1, typename data_type2>
__global__ static void add_kernel(const data_type1 *A, float *alpha, const data_type2 *B, float *beta, data_type1 *C, int size)
{
	int idx = threadIdx.x + blockIdx.x * blockDim.x;
	if (idx < size)
	{
...
cudaMatrix<data_type>::cudaMatrix(int size) : rows(size), cols(size)
{
	...
	if (err != cudaSuccess)
	{
		throw runtime_error("cudaMalloc failed! (code M0002)" + string(cudaGetErrorString(err)));
	}
	cudaMemset(data, 0, static_cast<size_t>(rows) * cols * sizeof(data_type));
}
...
cudaMatrix<data_type>::cudaMatrix(int rows, int cols, MatrixType type) : rows(rows), cols(cols)
{
	...
	if (err != cudaSuccess)
	{
		throw runtime_error("cudaMalloc failed! (code M0003)" + string(cudaGetErrorString(err)));
	}
	...
	case Random:
		...
		if (err != cudaSuccess)
		{
			throw runtime_error("cudaMalloc failed for curandState! (code M0005)" + string(cudaGetErrorString(err)));
		}
...
cudaMatrix<data_type>::cudaMatrix(int size, MatrixType type) : rows(size), cols(size)
{
	...
	if (err != cudaSuccess)
	{
		throw runtime_error("cudaMalloc failed! (code M0004)" + string(cudaGetErrorString(err)));
	}
	int threadsPerBlock = 0;
	int blocksPerGrid = 0;
	switch (type)
...
template <typename data_type>
template <typename T>
cudaMatrix<data_type>::cudaMatrix(const cudaMatrix<T> &others) : rows(others.rows), cols(others.cols)
{
	cudaError_t err = cudaMalloc((void **)&data, static_cast<size_t>(rows) * cols * sizeof(data_type));
	if (err != cudaSuccess)
	{
		throw runtime_error("cudaMalloc failed! (code M0006)");
	}
	if constexpr (is_same<data_type, T>::value)
	{
		cudaMemcpy(data, others.data, static_cast<size_t>(rows) * cols * sizeof(data_type), cudaMemcpyDeviceToDevice);
	}
	else
	{
		int threadsPerBlock = autoSetBlockSize(convert_kernel<data_type, T>);
		int blocksPerGrid = (rows * cols + threadsPerBlock - 1) / threadsPerBlock;
		convert_kernel<T, data_type><<<blocksPerGrid, threadsPerBlock>>>(others.data, data, rows * cols);
	}
}
...
void cudaMatrix<data_type>::subtract(const cudaMatrix<data_type> &B)
{
	...
	cudaMemcpy(beta, &b, sizeof(float), cudaMemcpyHostToDevice);
	int threadsPerBlock = autoSetBlockSize(add_kernel<data_type, data_type>);
	int blocksPerGrid = (rows * cols + threadsPerBlock - 1) / threadsPerBlock;
	add_kernel<data_type, data_type><<<blocksPerGrid, threadsPerBlock>>>(data, alpha, B.data, beta, data, rows * cols);
	cudaFree(alpha);
	cudaFree(beta);
}
...
template <typename data_type>
void cudaMatrix<data_type>::multiply(cudaMatrix<data_type> &B)
{ // 请不要管这里，这里是屎山
	if (rows == 1 && cols == 1)
	{
		cublasHandle_t handle;
...
template <typename data_type>
cudaMatrix<data_type> cudaMatrix<data_type>::multiply(const cudaMatrix &A, const cudaMatrix &B)
{ // 请不要管这里，这里是屎山
...
template <typename data_type>
template <typename T>
cudaMatrix<data_type> cudaMatrix<data_type>::operator*=(const T scalar) { return this->scalarMultiply(scalar); }
...
data_type cudaMatrix<data_type>::trace() const
{
	...
	reduce_sum<<<1, 512, 512 * sizeof(data_type)>>>(trace_array, d_result, size);
	data_type result = 0;
...
template <typename data_type>
template <typename T>
cudaMatrix<data_type> cudaMatrix<data_type>::dot(const T scalar, const cudaMatrix<data_type> &A) { return A.scalarMultiply(scalar); }

template <typename data_type>
template <typename T>
cudaMatrix<data_type> cudaMatrix<data_type>::dot(const cudaMatrix &A, const T scalar) { return A.scalarMultiply(scalar); }
